...
async def lifespan(app: FastAPI):
    logger.info("Starting LLM API lifespan: init LLMService")
    vdb = LLMService()
    app.state.vdb = vdb
    ...
    try:
        ...
        logger.exception("LLM ignored health-check failed on startup (ignored)")

    try:
        yield
    finally:
        logger.info("Shutting down LLM API: closing LLMService")
        try:
            await vdb.close()
        except Exception:
            logger.exception("Error closing LLMService")
...
async def create_collection(
    ...
    vdb: LLMService = Depends(get_llm),
):
    try:
